...
namespace GeneticSharp.Runner.UnityApp.Commons
{
    public class SinglePointCrossover : ICrossover
    {
        ...
        {
            IChromosome parent1 = parents[0]; //informações sobre um pai
            IChromosome parent2 = parents[1]; //informações sobre um pai
            ...
            if (RandomizationProvider.Current.GetDouble() <= crossoverProbability)
            {
                ...
                while (i < cutPoint)
                {
                    offspring1.ReplaceGene(i, parent2.GetGene(i)); //de forma random, são colocadas informações do segundo pai no primeiro filho
                    offspring2.ReplaceGene(i, parent1.GetGene(i)); //de forma random, são colocadas informações do primeiro pai no segundo filho
                    i++;
                }
            }
